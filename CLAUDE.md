...
- `mcp-app-frontend` and `generic-rag` are the two components with cross-component wiring that is *not* just a host name: the widget's `VITE_BASE_URL` must equal the `MCP_APP_ORIGIN` both backends advertise (a mismatch makes the host iframe 404 on the widget's assets), and `generic-rag` needs a Postgres database of its own rather than the backends' `PGVECTOR_DATABASE`.
